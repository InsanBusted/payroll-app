    ...
    @if (!$employee)
        ...
        <div class="flex flex-col items-center justify-center py-24 gap-5">
            ...
            <div class="text-center">
                ...
                <p class="text-sm text-slate-400 mt-1">Hubungi administrator untuk menautkan akun Anda ke data karyawan.
                </p>
            </div>
    ...
    @else
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center gap-5">
                <div
                    class="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-400 flex items-center justify-center text-2xl font-extrabold text-white flex-shrink-0 shadow">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-2 mt-2">
                        @if ($employee->jabatan)
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-700">{{ $employee->jabatan->nama }}</span>
                        @endif
                        @if ($employee->area)
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700">{{ $employee->area->nama }}</span>
                        @endif
                        @if ($employee->ptkpStatus)
                            <span
                                ...
                                {{ $employee->ptkpStatus->status }}</span>
                        @endif
                    </div>
                </div>
                ...
                @if ($periodes->isNotEmpty())
                    <form method="GET" action="{{ route('staff.kinerja') }}" class="flex-shrink-0">
                        ...
                        @endphp
                        <label
                            class="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Pilih
                            Periode</label>
        ...
        @if (!$kinerja)
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-16 text-center text-slate-400">
                <svg class="w-12 h-12 mx-auto mb-3 opacity-30" fill="none" stroke="currentColor" stroke-width="1.5"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        @else
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
                <div
                    class="bg-gradient-to-br from-indigo-500 to-violet-500 rounded-2xl p-6 text-white shadow-lg shadow-indigo-200">
                    <p class="text-indigo-100 text-[11px] font-semibold uppercase tracking-wider">Gaji Bersih —
                        {{ $periodeTeks }}</p>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col justify-center">
                    ...
                    <p class="text-2xl font-extrabold text-red-500 mt-1">Rp
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div class="divide-y divide-slate-50">
                        @foreach ([['Gaji Pokok', $pendapatan['gaji_pokok'] ?? 0], ['Tunjangan Kerapihan', $pendapatan['tunjangan_kerapihan'] ?? 0], ['SRP', $pendapatan['srp'] ?? 0], ['Grosir', $pendapatan['grosir'] ?? 0], ['Aksesoris', $pendapatan['aksesoris'] ?? 0], ['Bonus', $pendapatan['bonus'] ?? 0]] as [$label, $val])
                        ...
                        <div class="flex items-center justify-between px-6 py-3.5 bg-emerald-50">
                            ...
                            <span class="text-sm font-bold text-emerald-700">Rp
                ...
                <div class="space-y-5">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div class="divide-y divide-slate-50">
                            @foreach ([['Absensi', $potongan['absensi'] ?? 0], ['BPJS TK', $potongan['bpjstk'] ?? 0], ['PPh 21', $potongan['pph21'] ?? 0]] as [$label, $val])
                                <div class="flex items-center justify-between px-6 py-3.5">
                                    <span class="text-sm text-slate-600">{{ $label }}</span>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                        ...
                        <div class="flex items-center gap-4">
                            <div
                                class="w-16 h-16 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center flex-shrink-0">
